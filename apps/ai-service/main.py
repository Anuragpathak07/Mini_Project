...
async def analyze_radiology(file: UploadFile = File(...)):
    """Receives a patient's medical scan/x-ray and provides a clinical AI analysis for doctors."""
    if not file.filename:
        raise HTTPException(status_code=400, detail="No file uploaded")
    if not model:
        raise HTTPException(status_code=500, detail="GEMINI_API_KEY is not configured.")
    ...
    if suffix.lower() not in ['.png', '.jpg', '.jpeg']:
        raise HTTPException(status_code=400, detail="Only images (PNG, JPG) are supported for radiology analysis.")
    ...
    try:
        ...
        prompt = """
        You are a highly advanced AI Radiology Assistant. You are analyzing an imaging scan on behalf of a professional doctor.
        Please analyze this image and provide:
        1. **Modality**: What kind of scan is this? (X-ray, MRI, etc.)
        2. **Observations**: Detail any specific physiological observations, anomalies, or points of interest. 
        3. **Preliminary AI Impression**: Provide a clinical impression.
        
        IMPORTANT: End with a clear disclaimer: "DISCLAIMER: This analysis is generated by AI and must be independently verified by a qualified radiologist."
        """
